# ==============================================================================
# LIVE Pipeline Tests for ncschooldata
# ==============================================================================
#
...
test_that("North Carolina DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add North Carolina DOE URL and verify HTTP 200")
})
...
test_that("Can download North Carolina enrollment data file", {
  skip_if_offline()
...
test_that("Can parse North Carolina enrollment file", {
  skip_if_offline()
...
test_that("ncschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- ncschooldata::get_available_years()
  ...
  tryCatch({
    raw <- ncschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- ncschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- ncschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- ncschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- ncschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- ncschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- ncschooldata:::get_cache_path(2024, "enrollment")
