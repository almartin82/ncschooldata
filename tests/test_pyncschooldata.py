...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = nc.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pyncschooldata as nc
        assert hasattr(nc, 'fetch_enr')
        assert hasattr(nc, 'fetch_enr_multi')
        assert hasattr(nc, 'tidy_enr')
        assert hasattr(nc, 'get_available_years')
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = nc.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = nc.fetch_enr(years['max_year'])
        # Total enrollment in North Carolina varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # NC should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = nc.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = nc.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pyncschooldata as nc
        years = get_test_years()
        df_single = nc.fetch_enr(years['max_year'])
        df_multi = nc.fetch_enr_multi([years['max_year']])
